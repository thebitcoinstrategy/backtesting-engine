...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .tool-category.collapsed .arrow {
            transform: rotate(-90deg);
        }
        .tool-category.collapsed .tool-category-body {
            display: none;
        }
        ...
        .lw-drawings-header #lw-drawing-count {
            background: var(--bg-elevated);
            color: var(--text-muted);
            font-size: 0.78em;
            padding: 1px 8px;
        ...
        .lw-drawing-item:hover {
            background: var(--bg-elevated);
            color: var(--text);
        }
        .lw-drawing-item.selected {
            background: rgba(68, 138, 255, 0.08);
            color: var(--accent);
        }
        .lw-drawing-item .lw-drawing-color {
            width: 8px;
            height: 8px;
            border-radius: 50%;
            flex-shrink: 0;
        }
        .lw-drawing-item .lw-drawing-name {
            flex: 1;
            margin: 0 8px;
            white-space: nowrap;
            overflow: hidden;
        ...
        .lw-text-editor-overlay.visible {
            display: flex;
        }
        .lw-text-editor-modal {
            background: var(--bg-elevated);
            border: 1px solid var(--border);
            border-radius: 12px;
            padding: 20px;
        ...
        .lw-text-editor-modal h3 {
            margin: 0 0 12px;
            font-size: 0.95em;
            color: var(--text);
        }
        .lw-text-editor-modal textarea {
            width: 100%;
            min-height: 80px;
            background: var(--bg-surface);
            border: 1px solid var(--border);
        ...
        .lw-text-editor-modal textarea:focus {
            outline: none;
            border-color: var(--accent);
        }
        .lw-text-editor-hint {
        ...
        .lw-text-editor-actions {
            display: flex;
            gap: 8px;
            justify-content: flex-end;
        }
        .lw-btn-cancel, .lw-btn-save {
            padding: 6px 16px;
            border-radius: 6px;
            border: 1px solid var(--border);
            font-family: 'DM Sans', sans-serif;
        ...
        @media (max-width: 900px) {
            .lw-tools-panel, .lw-drawings-panel { display: none; }
            .lw-status-bar { border-radius: 0 0 12px 12px; }
            #lw-chart-container { border-radius: 12px 12px 0 0 !important; }
        }
        ...
        <div class="panel" id="results-panel">
            {% if chart %}
                ...
                <div id="livechart-tab" style="display:none">
                    ...
                    <aside class="lw-tools-panel">
                        <div class="lw-tools-header">
                            <span>Drawing Tools</span>
                            <button class="lw-clear-btn" onclick="clearAllDrawings()">Clear All</button>
                        </div>
                        <div class="tool-category">
                            <div class="tool-category-header" onclick="this.parentElement.classList.toggle('collapsed')">
                                <span>Trend Lines</span><span class="arrow">&#9660;</span>
                            </div>
                            <div class="tool-category-body">
                                <div class="tool-subcategory-header">Lines</div>
                                <div class="tool-item" data-tool="trend-line" onclick="selectDrawingTool('trend-line',this)">Trend Line</div>
                                <div class="tool-item" data-tool="horizontal-line" onclick="selectDrawingTool('horizontal-line',this)">Horizontal Line</div>
                                <div class="tool-item" data-tool="vertical-line" onclick="selectDrawingTool('vertical-line',this)">Vertical Line</div>
                                <div class="tool-item" data-tool="ray" onclick="selectDrawingTool('ray',this)">Ray</div>
                                <div class="tool-item" data-tool="extended-line" onclick="selectDrawingTool('extended-line',this)">Extended Line</div>
                                <div class="tool-item" data-tool="cross-line" onclick="selectDrawingTool('cross-line',this)">Cross Line</div>
                                <div class="tool-item" data-tool="horizontal-ray" onclick="selectDrawingTool('horizontal-ray',this)">Horizontal Ray</div>
                                <div class="tool-item" data-tool="info-line" onclick="selectDrawingTool('info-line',this)">Info Line</div>
                                <div class="tool-item" data-tool="trend-angle" onclick="selectDrawingTool('trend-angle',this)">Trend Angle</div>
                                <div class="tool-subcategory-header">Channels</div>
                                <div class="tool-item" data-tool="parallel-channel" onclick="selectDrawingTool('parallel-channel',this)">Parallel Channel</div>
                                <div class="tool-item" data-tool="disjoint-channel" onclick="selectDrawingTool('disjoint-channel',this)">Disjoint Channel</div>
                                <div class="tool-item" data-tool="flat-top-bottom" onclick="selectDrawingTool('flat-top-bottom',this)">Flat Top/Bottom</div>
                                <div class="tool-item" data-tool="regression-trend" onclick="selectDrawingTool('regression-trend',this)">Regression Trend</div>
                            </div>
                        </div>
                        <div class="tool-category">
                            <div class="tool-category-header" onclick="this.parentElement.classList.toggle('collapsed')">
                                <span>Fibonacci</span><span class="arrow">&#9660;</span>
                            </div>
                            <div class="tool-category-body">
                                <div class="tool-item" data-tool="fib-retracement" onclick="selectDrawingTool('fib-retracement',this)">Fib Retracement</div>
                                <div class="tool-item" data-tool="fib-extension" onclick="selectDrawingTool('fib-extension',this)">Fib Extension</div>
                                <div class="tool-item" data-tool="fib-channel" onclick="selectDrawingTool('fib-channel',this)">Fib Channel</div>
                            </div>
                        </div>
                        <div class="tool-category">
                            <div class="tool-category-header" onclick="this.parentElement.classList.toggle('collapsed')">
                                <span>Shapes</span><span class="arrow">&#9660;</span>
                            </div>
                            <div class="tool-category-body">
                                <div class="tool-item" data-tool="rectangle" onclick="selectDrawingTool('rectangle',this)">Rectangle</div>
                                <div class="tool-item" data-tool="rotated-rectangle" onclick="selectDrawingTool('rotated-rectangle',this)">Rotated Rectangle</div>
                                <div class="tool-item" data-tool="circle" onclick="selectDrawingTool('circle',this)">Circle</div>
                                <div class="tool-item" data-tool="ellipse" onclick="selectDrawingTool('ellipse',this)">Ellipse</div>
                                <div class="tool-item" data-tool="triangle" onclick="selectDrawingTool('triangle',this)">Triangle</div>
                                <div class="tool-item" data-tool="arc" onclick="selectDrawingTool('arc',this)">Arc</div>
                                <div class="tool-item" data-tool="price-range" onclick="selectDrawingTool('price-range',this)">Price Range</div>
                            </div>
                        </div>
                        <div class="tool-category">
                            <div class="tool-category-header" onclick="this.parentElement.classList.toggle('collapsed')">
                                <span>Lines &amp; Curves</span><span class="arrow">&#9660;</span>
                            </div>
                            <div class="tool-category-body">
                                <div class="tool-item" data-tool="arrow" onclick="selectDrawingTool('arrow',this)">Arrow</div>
                                <div class="tool-item" data-tool="curve" onclick="selectDrawingTool('curve',this)">Curve</div>
                                <div class="tool-item" data-tool="double-curve" onclick="selectDrawingTool('double-curve',this)">Double Curve</div>
                                <div class="tool-item" data-tool="polyline" onclick="selectDrawingTool('polyline',this)">Polyline</div>
                                <div class="tool-item" data-tool="path" onclick="selectDrawingTool('path',this)">Path</div>
                            </div>
                        </div>
                    </aside>
                    ...
                    <div class="lw-chart-area">
                        <div id="lw-chart-container"
                             style="height:600px;overflow:hidden;border:1px solid var(--border)">
                        </div>
                        <div class="lw-status-bar">
                            <span>Tool: <span id="lw-status-tool">None</span></span>
                            <span>State: <span id="lw-status-state">Idle</span></span>
                        </div>
                    </div>

                    <!-- Right: Active Drawings List -->
                    <aside class="lw-drawings-panel">
                        <div class="lw-drawings-header">
                            <span>Drawings</span>
                            <span id="lw-drawing-count">0</span>
                        </div>
                        <div id="lw-drawing-list">
                            <div class="lw-empty-state">No drawings yet</div>
                        </div>
                    </aside>
                </div>
                <script>
...
function switchChartTab(tab, btn) {
    ...
    lw.style.display = tab === 'livechart' ? 'flex' : 'none';
...
function loadLWChart() {
    ...
    if (!LCD || !LCD.DrawingManager) {
        console.warn('lightweight-charts-drawing not loaded');
        return;
    }

    var dm = new LCD.DrawingManager();
    dm.attach(chart, priceSeries, container);

    window.__lwChart = chart;
    window.__lwSeries = priceSeries;
    window.__lwDM = dm;
    window.__lwContainer = container;
    window.__lwCurrentTool = null;
    window.__lwAnchors = [];
    window.__lwDrawingId = 0;
    window.__lwPreview = null;

    // Tool type → class name mapping
    var toolClasses = {
        'trend-line': 'TrendLine', 'horizontal-line': 'HorizontalLine',
        'vertical-line': 'VerticalLine', 'ray': 'Ray', 'arrow': 'Arrow',
        'extended-line': 'ExtendedLine', 'cross-line': 'CrossLine',
        'info-line': 'InfoLine', 'trend-angle': 'TrendAngle',
        'horizontal-ray': 'HorizontalRay', 'rectangle': 'Rectangle',
        'rotated-rectangle': 'RotatedRectangle', 'circle': 'Circle',
        'triangle': 'Triangle', 'price-range': 'PriceRange',
        'ellipse': 'Ellipse', 'arc': 'Arc', 'path': 'Path',
        'polyline': 'Polyline', 'curve': 'Curve', 'double-curve': 'DoubleCurve',
    ...
    // Anchor counts per tool type
    var anchorCounts = {
        'horizontal-line': 1, 'vertical-line': 1, 'cross-line': 1,
        'triangle': 3, 'flat-top-bottom': 3, 'disjoint-channel': 3, 'fib-channel': 3,
        'path': -1, 'polyline': -1
    };
    // Default is 2 anchors for anything not listed

    // Tool colors for the drawings panel
    var toolColors = {
        'trend-line': '#448aff', 'horizontal-line': '#448aff', 'vertical-line': '#448aff',
        'ray': '#448aff', 'extended-line': '#448aff', 'cross-line': '#448aff',
        'horizontal-ray': '#448aff', 'info-line': '#448aff', 'trend-angle': '#448aff',
        'parallel-channel': '#26a69a', 'disjoint-channel': '#26a69a',
        'flat-top-bottom': '#26a69a', 'regression-trend': '#26a69a',
        'fib-retracement': '#ffd54f', 'fib-extension': '#ffd54f', 'fib-channel': '#ffd54f',
        'rectangle': '#ab47bc', 'rotated-rectangle': '#ab47bc', 'circle': '#ab47bc',
        'ellipse': '#ab47bc', 'triangle': '#ab47bc', 'arc': '#ab47bc', 'price-range': '#ab47bc',
        'arrow': '#ef5350', 'curve': '#ef5350', 'double-curve': '#ef5350',
        'polyline': '#ef5350', 'path': '#ef5350'
    };

    // Pretty names for display
    var toolNames = {};
    var items = document.querySelectorAll('.tool-item[data-tool]');
    for (var i = 0; i < items.length; i++) {
        toolNames[items[i].getAttribute('data-tool')] = items[i].textContent.trim();
    }

    // Convert chart click params to anchor
    function clickToAnchor(param) {
        if (!param.time || !param.point) return null;
        var price = priceSeries.coordinateToPrice(param.point.y);
        if (price === null || isNaN(price)) return null;
        return { time: param.time, price: price };
    }

    // Create a drawing from tool type and anchors
    function createDrawing(toolType, anchors) {
        var className = toolClasses[toolType];
        if (!className || !LCD[className]) {
            console.warn('Unknown drawing class:', toolType, className);
            return null;
        }
        var id = 'drawing-' + (++window.__lwDrawingId);
        var color = toolColors[toolType] || '#448aff';
        var style = { color: color, lineWidth: 2 };
        try {
            var drawing = new LCD[className](id, anchors, style);
            dm.addDrawing(drawing);
            drawing._toolType = toolType;
            return drawing;
        } catch (e) {
            console.warn('Failed to create drawing:', toolType, e);
            return null;
        }
    }

    // Chart click handler for anchor placement
    chart.subscribeClick(function(param) {
        if (!window.__lwCurrentTool) return;
        var anchor = clickToAnchor(param);
        if (!anchor) return;

        var toolType = window.__lwCurrentTool;
        var needed = anchorCounts[toolType] || 2;

        // Variable-anchor tools (path, polyline): collect until double-click
        if (needed === -1) {
            window.__lwAnchors.push(anchor);
            updateStatus(toolType, 'Anchor ' + window.__lwAnchors.length + ' placed (dbl-click to finish)');
            // Remove preview
            if (window.__lwPreview) {
                try { dm.removeDrawing(window.__lwPreview.id); } catch(e) {}
                window.__lwPreview = null;
            }
            // Show preview with current anchors
            if (window.__lwAnchors.length >= 2) {
                window.__lwPreview = createDrawing(toolType, window.__lwAnchors.slice());
            }
            return;
        }

        window.__lwAnchors.push(anchor);
        updateStatus(toolType, 'Anchor ' + window.__lwAnchors.length + '/' + needed);
        ...
        if (window.__lwAnchors.length >= needed) {
            // Create final drawing
            createDrawing(toolType, window.__lwAnchors.slice());
            window.__lwAnchors = [];
            updateStatus(toolType, 'Drawing placed! Click to draw another');
            updateDrawingList();
        }
    });

    // Double-click to finish variable-anchor tools
    container.addEventListener('dblclick', function() {
        if (!window.__lwCurrentTool) return;
        var needed = anchorCounts[window.__lwCurrentTool] || 2;
        if (needed !== -1) return;
        if (window.__lwAnchors.length < 2) return;

        // Remove preview, create final
        if (window.__lwPreview) {
            try { dm.removeDrawing(window.__lwPreview.id); } catch(e) {}
            window.__lwPreview = null;
        }
        createDrawing(window.__lwCurrentTool, window.__lwAnchors.slice());
        window.__lwAnchors = [];
        updateStatus(window.__lwCurrentTool, 'Drawing placed! Click to draw another');
        updateDrawingList();
    });

    // Listen for DrawingManager events
    dm.on('drawing:added', function() { updateDrawingList(); });
    dm.on('drawing:removed', function() { updateDrawingList(); });
    dm.on('drawing:selected', function(e) { highlightDrawingItem(e.drawingId); });
    dm.on('drawing:deselected', function() { highlightDrawingItem(null); });

    // Keyboard shortcuts
    document.addEventListener('keydown', function(e) {
        if (!window.__lwDM) return;
        if (e.key === 'Escape') {
            // Cancel in-progress drawing
            if (window.__lwAnchors && window.__lwAnchors.length > 0) {
                if (window.__lwPreview) {
                    try { dm.removeDrawing(window.__lwPreview.id); } catch(ex) {}
                    window.__lwPreview = null;
                }
                window.__lwAnchors = [];
                updateStatus(window.__lwCurrentTool || 'None', 'Cancelled');
            }
            // Deselect tool
            deselectTool();
        }
        if (e.key === 'Delete' || e.key === 'Backspace') {
            var sel = dm.getSelectedDrawing ? dm.getSelectedDrawing() : null;
            if (sel && !e.target.matches('input,textarea,select')) {
                e.preventDefault();
    ...
    // Status bar helpers
    function updateStatus(tool, state) {
        var toolEl = document.getElementById('lw-status-tool');
        var stateEl = document.getElementById('lw-status-state');
        if (toolEl) toolEl.textContent = toolNames[tool] || tool || 'None';
        if (stateEl) stateEl.textContent = state || 'Idle';
    }

    // Update the drawings list panel
    function updateDrawingList() {
        var list = document.getElementById('lw-drawing-list');
        var countEl = document.getElementById('lw-drawing-count');
        if (!list) return;
        var drawings = dm.getAllDrawings ? dm.getAllDrawings() : [];
        if (countEl) countEl.textContent = drawings.length;
        if (drawings.length === 0) {
            list.innerHTML = '<div class="lw-empty-state">No drawings yet</div>';
            return;
        }
        var html = '';
        for (var i = 0; i < drawings.length; i++) {
            var d = drawings[i];
            var tt = d._toolType || d.type || 'unknown';
            var name = toolNames[tt] || tt;
            var color = toolColors[tt] || '#448aff';
            var sel = dm.getSelectedDrawing && dm.getSelectedDrawing() === d ? ' selected' : '';
            html += '<div class="lw-drawing-item' + sel + '" data-id="' + d.id + '" onclick="selectLWDrawing(\'' + d.id + '\')">'
                + '<span class="lw-drawing-color" style="background:' + color + '"></span>'
                + '<span class="lw-drawing-name">' + name + '</span>'
                + '<button class="lw-drawing-del" onclick="event.stopPropagation();deleteLWDrawing(\'' + d.id + '\')" title="Delete">&#215;</button>'
                + '</div>';
        }
        list.innerHTML = html;
    }

    function highlightDrawingItem(id) {
        var items = document.querySelectorAll('.lw-drawing-item');
        for (var i = 0; i < items.length; i++) {
    ...
    window.selectLWDrawing = function(id) {
        if (dm.selectDrawing) dm.selectDrawing(id);
    };
    window.deleteLWDrawing = function(id) {
        dm.removeDrawing(id);
        updateDrawingList();
    };
    window.clearAllDrawings = function() {
        if (dm.clearAll) dm.clearAll();
        updateDrawingList();
    };

    window.updateDrawingList = updateDrawingList;
    window.updateLWStatus = updateStatus;
}

function selectDrawingTool(toolType, el) {
    // Deselect previous
    var prev = document.querySelector('.tool-item.active');
    if (prev) prev.classList.remove('active');

    // If clicking same tool, deselect
    if (window.__lwCurrentTool === toolType) {
        deselectTool();
        return;
    }

    el.classList.add('active');
    window.__lwCurrentTool = toolType;
    window.__lwAnchors = [];
    if (window.__lwPreview && window.__lwDM) {
        try { window.__lwDM.removeDrawing(window.__lwPreview.id); } catch(e) {}
        window.__lwPreview = null;
    }
    if (window.__lwDM && window.__lwDM.deselectAll) window.__lwDM.deselectAll();

    var statusTool = document.getElementById('lw-status-tool');
    var statusState = document.getElementById('lw-status-state');
    if (statusTool) statusTool.textContent = el.textContent.trim();
    if (statusState) statusState.textContent = 'Click chart to place anchors';
}
...
function deselectTool() {
    ...
    var statusTool = document.getElementById('lw-status-tool');
    var statusState = document.getElementById('lw-status-state');
    if (statusTool) statusTool.textContent = 'None';
    if (statusState) statusState.textContent = 'Idle';
}
