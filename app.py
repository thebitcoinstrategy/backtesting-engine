        ...
        <div class="panel" id="results-panel">
            {% if chart %}
                ...
                <table class="metrics-table">
                    ...
                    <tr><td class="m-label">Trades <span class="m-info" data-tip="Number of position changes.&#10;Count of buy/sell signals">ⓘ</span></td><td class="m-val">{{ best.trades }}{% if ls %}<span class="m-ls"><span class="ls-l">Long {{ ls.long.trades }}</span> · <span class="ls-s">Short {{ ls.short.trades }}</span></span>{% endif %}</td><td class="m-val muted">&mdash;</td></tr>
                    <tr><td class="m-label">Win Rate <span class="m-info" data-tip="Percentage of trades that made money.&#10;Formula: winning trades ÷ total trades">ⓘ</span></td><td class="m-val">{{ "%.1f"|format(best.win_rate) }}%{% if ls %}<span class="m-ls"><span class="ls-l">Long {{ "%.1f"|format(ls.long.win_rate) }}%</span> · <span class="ls-s">Short {{ "%.1f"|format(ls.short.win_rate) }}%</span></span>{% endif %}</td><td class="m-val muted">&mdash;</td></tr>
